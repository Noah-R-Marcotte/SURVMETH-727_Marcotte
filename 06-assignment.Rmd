...
```{r}
pop <- hist_pop[2:10, -3]
colnames(pop) <- c("Census(Year)", "Grand Boulevard","%±")
print(pop)
```
...
```{r}
for(i in 1:length(East2)) {
url[i] <- paste("https://en.wikipedia.org/wiki/", East2[i],
                sep = "")
  }
url
```
...
```{r}
name=vector(length = length(East2))
for(i in 1:length(East2)){
name[i]=strsplit(East2,split=',')[[i]][1]
}
name=gsub("_", " ", name)
```
...
```{r}
pops=pop
colnames(pops) <- c("Census(Year)", "Grand Boulevard","%±")
for(i in 1:length(East2)) {
  url2 <- read_html(url[i])
  tables2 <- html_table(url2)
  hist_pop2[[i]]<- tables2[[2]]
  pop2 <- hist_pop2[[i]][2:10, -c(1,3)]
  colnames(pop2) <- c(name[i],"%±")
  pops=cbind(pops,pop2)
}
